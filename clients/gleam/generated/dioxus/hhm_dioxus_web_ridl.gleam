...
pub const service = "hhm-dioxus-web"
...
pub fn health_decoder() -> decode.Decoder(Health) {
  ...
  use ui <- decode.field("ui", decode.string)
  decode.success(Health(status, ui))
}

pub fn health_to_json(value: Health) -> json.Json {
  json.object([
    #("status", json.string(value.status)),
    #("ui", json.string(value.ui)),
  ])
}
...
pub fn operations() -> List(OperationInfo) {
  [
    ...
    OperationInfo("realtime", "/ws", ["GET"], Direct),
  ]
}
